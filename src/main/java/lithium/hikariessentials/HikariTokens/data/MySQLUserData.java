...
public class MySQLUserData {
    ...
    public boolean exists(UUID uuid) {
        try {
            PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement("SELECT * FROM `user` WHERE (UUID=?)");
    ...
    public void createPlayer(Player p) {
        try {
            if (!this.exists(p.getUniqueId())) {
                PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement(
                        "INSERT INTO `user` " +
    ...
    public static void setIgnore(UUID uuid, boolean ignore) {
        try {
            PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement("UPDATE `user` SET Ignore_Pay=? WHERE (UUID=?)");
    ...
    public static void setTokens(UUID uuid, int amount) {
        try {
            PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement("UPDATE `user` SET Tokens=? WHERE (UUID=?)");
            statement.setInt(1, amount);
            statement.setString(2, uuid.toString());
            statement.executeUpdate();
        } catch (SQLException e) {
    ...
    public static double getTokensDoubleByName(String player) {
        try {
            PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement("SELECT * FROM `user` WHERE (Name=?)");
    ...
    public static TreeMap<String, Integer> getTop10() {
        ...
        try {
            PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement("SELECT * FROM `user` ORDER BY 'Tokens' DESC LIMIT " +
    ...
    public static Double getServerTotalTokens() {
        try {
            PreparedStatement statement = HikariEssentialsToken.getMysql().getConnection().prepareStatement("SELECT * FROM `user`");
